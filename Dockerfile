...
RUN apt update -y && apt install -y openssh-server && rm -rf /var/lib/apt/lists/* 
RUN mkdir -p ~/.ssh && chmod 700 ~/.ssh && cat /id_rsa.pub  >> ~/.ssh/authorized_keys && chmod 600 ~/.ssh/authorized_keys
###finalizando ssh YDTG
